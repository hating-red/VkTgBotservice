...
⏰ <b>Время:</b> ${order.startTime && order.hours ? 
  `с ${order.startTime} до ${this.calculateEndTime(order.startTime, order.hours)} (${order.hours} ч.)` : 
  order.startTime ? `${order.startTime}${order.hours ? ` (${order.hours} ч.)` : ''}` : 
  'не указано'}
